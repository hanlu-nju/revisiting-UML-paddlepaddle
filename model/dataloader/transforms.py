...
class Lighting(object):
    ...
    def __call__(self, img):
        ...
        rgb = self.eigvec.type_as(img).clone().mul(alpha.reshape((1, 3)).expand(
            3, 3)).mul(self.eigval.reshape((1, 3)).expand(3, 3)).sum(1).squeeze()
        return img.add(rgb.reshape((3, 1, 1)).expand_as(img))


class ERandomCrop:

    def __init__(self, imgsize, min_covered=0.1, aspect_ratio_range=(3.0 / 
        4, 4.0 / 3), area_range=(0.1, 1.0), max_attempts=10):
        ...
        self.resize_method = Resize((imgsize, imgsize), interpolation=Image
            .BICUBIC)
...
class ECenterCrop:
    ...
    def __call__(self, img):
        ...
        img = img.crop((crop_left, crop_top, crop_left + crop_width, 
