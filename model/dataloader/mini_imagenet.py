...
from model.dataloader.base import ROOT_DIRS
from model.dataloader.base import search_dir
THIS_PATH = osp.dirname(__file__)
ROOT_PATH = osp.abspath(osp.join(THIS_PATH, '..', '..'))
ROOT_PATH2 = osp.abspath(osp.join(THIS_PATH, '..', '..', '..', '..'))
data_dirs = ['miniimagenet', 'mini-imagenet']
...
class MiniImageNet(BaseDataset):
    ...
    def __init__(self, setname, unsupervised, args, augment='none'):
        self.IMAGE_PATH1 = search_dir([osp.join(args.data_root, di,
            'images') for di in data_dirs])
